...
def classify_text(message):
    ...
    prompt = f"""Classify the following text of news articles as either legitimate (true information) or fake (false information).
    Message: '{truncated_message}'.
    The output should only contain two words: legitimate or fake."""
    ...
    try:
        ...
        confidence = 1.0 if prediction == 'legitimate' else 0.0
...
def main():
    ...
    for _, row in tqdm(dataset.iterrows(), total=len(dataset)):
        ...
        if prediction is not None:
            predictions.append(1 if prediction == 'legitimate' else 0)
