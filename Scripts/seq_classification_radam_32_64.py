...
def compute_objective(trial):
    ...
    training_args = TrainingArguments(
        output_dir=f'./results_{opt_model_name}_radam',
        save_total_limit=1,
        report_to=None,
...
study = optuna.create_study(
    ...
    load_if_exists=True,
    study_name=f'{opt_model_name}-study-RAdam',
    storage=f'sqlite:///{opt_model_name}-study-RAdam.db'
)
...
model = model_init()
optimizer = RAdam(model.parameters(), lr=best['learning_rate'])
...
with open("results_seq_classification_radam.txt", "a") as arquivo:
    arquivo.write(f"eval radam {opt_model_name}:\n")
...
trainer.save_model(output_dir=f'./results_{opt_model_name}_radam')