...
def classify_text(message):
    ...
    prompt = f"""Classify the following text of banking regulatory risk from different departments of Banco do Brasil as either relevant (impacting departmental compliance and operations) or not relevant (not impacting departmental compliance).
    Message: '{truncated_message}'.
    The output should only contain two words: relevant or not relevant."""
    ...
    try:
        ...
        confidence = 1.0 if prediction == 'relevant' else 0.0
...
def main():
    ...
    for _, row in tqdm(dataset.iterrows(), total=len(dataset)):
        ...
        if prediction is not None:
            predictions.append(1 if prediction == 'relevant' else 0)
