...
def classify_text(message):
    ...
    prompt = f"""Classify the following web comment as either hate speech or not hate speech.
    Message: '{truncated_message}'.
    The output should only contain two words: hate or not hate."""
    ...
    try:
        ...
        confidence = 1.0 if prediction == 'hate' else 0.0
...
def main():
    ...
    for _, row in tqdm(dataset.iterrows(), total=len(dataset)):
        ...
        if prediction is not None:
            predictions.append(1 if prediction == 'hate' else 0)
